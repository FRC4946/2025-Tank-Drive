...
public class ExampleCommand extends Command {
  ...
  public void execute() {
    if (Controller.getRawAxis(XboxController.Axis.kLeftY.value) > 0.1) {
      m_subsystem.leftWheels(Controller.getRawAxis(XboxController.Axis.kLeftY.value));
    } else if (Controller.getRawAxis(XboxController.Axis.kRightY.value) > 0.1) {
      m_subsystem.rightWheels(Controller.getRawAxis(XboxController.Axis.kRightY.value));
    } else if (Controller.getRawAxis(XboxController.Axis.kLeftY.value) < -0.1) {
      m_subsystem.leftWheels(Controller.getRawAxis(XboxController.Axis.kLeftY.value));
    } else if (Controller.getRawAxis(XboxController.Axis.kRightY.value) < -0.1) {
      m_subsystem.rightWheels(Controller.getRawAxis(XboxController.Axis.kRightY.value));
    } else {
      m_subsystem.rightWheels(0);
      m_subsystem.leftWheels(0);
    }
}
